// spsc_ring.cpp
// Single-producer / single-consumer ring buffer with "payload first, header last" commit.
// Fixes UB from casting non-atomic memory to atomic, adds bounds checks, validates mode,
// and provides portable wait/wake with optional ARM WFE/SEV.
// Adds C++20 [[likely]] / [[unlikely]] to guide branch prediction for mode dispatch.
//
// Build (Linux/Debian):
...
// -------- CRC16 (CCITT-FALSE) --------
static std::uint16_t crc16_ccitt_false(const std::uint8_t* data, std::size_t len) {
...
#if (defined(USE_ARM_SEV_WFE) && USE_ARM_SEV_WFE) && (defined(__aarch64__) || defined(__arm__))
                        ...
                        std::uint8_t gen_id)
{
    ...
    if (m > (std::uint8_t)Mode::Stop) return false;

    DataPacket& pkt = rb.data[rb.head];

    // ACQUIRE here prevents reordering around the "slot free" observation if code evolves.
...
static std::uint64_t girnos_worker(RingBuffer& rb) {
    ...
    for (;;) {
        ...
        // 1) ACQUIRE: observe header commit, then safely read payload written-before release store.
        ...
        switch ((Mode)h.mode) {
            case Mode::Raw: [[likely]] {
                apdoroti_raw(pkt.payload, h.length);
                break;
            }
            case Mode::Compressed: [[unlikely]] {
                apdoroti_unzip(pkt.payload, h.length);
                break;
            }
            case Mode::Stop: [[unlikely]] {
...
int main() {
    ...
    static constexpr std::uint32_t N = 200000;
    ...
    std::printf("produced=%llu consumed=%llu worker_done=%s\n",
                (unsigned long long)produced,
                (unsigned long long)c,
                worker_done.load(std::memory_order_relaxed) ? "true" : "false");
