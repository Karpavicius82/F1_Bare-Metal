...
// 4. Vector Filtering: accumulate tp/xp in __m256 and reduce periodically (amortized filter cost).
...
// Vector-accumulating kernel: no per-block scalar reduction (filter is amortized)
static inline void kernel_Pcore_v7_block_vec(const float* t_ptr, const float* x_ptr,
...
ThreadResult worker_main(const EngineConfig& cfg, WorkerShared* sh, int tid) {
    if (cfg.pin_workers) pin_to_pcore(tid);
    ...
    for (uint64_t i = 0; i < N; ++i) {
        ...
        kernel_Pcore_v7_block_vec(n->t + 8, n->x + 8, gamma, g_beta, perm_ctrl, otv, oxv);
    ...
    if (N > last_scan) sh->progress.fetch_add(N - last_scan, std::memory_order_relaxed);
...
int main(int argc, char** argv) {
    ...
    if (argc > 2) cfg.nodes_per_thread = (uint64_t)std::strtoull(argv[2], nullptr, 10); // test-friendly
    ...
    std::cout << " Ring: " << cfg.ring << "  RefreshMask: 0x" << std::hex << cfg.refresh_mask << std::dec << "\n";
    ...
    for (auto& r : results) { total_nps += r.nps; total_chk += r.checksum; }
    ...
    std::cout << "  Throughput(eq): " << std::setprecision(2) << gb_s << " GB/s\n";
